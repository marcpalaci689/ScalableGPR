...
class tensor_grid:
    ...
    def SKI(self,interpolation = 'linear'):
        ...
        if interpolation == 'ModifiedSheppard':
            #self.W = np.zeros((self.N,len(self.X)))
            row_ind = []
            col_ind = [] 
            weight  = []          
...
if __name__ == '__main__':
    ...
    grid.K = grid.Kd[0]
    K_SKI = grid.W.dot((grid.W.dot(grid.K)).T).T    + (noise**2)*np.eye(1000)  
    K = Gaussian(x,x,0,-4,10)
    
    
    
    grid1 = tensor_grid(x,[40])   
    grid1.generate(parameters)
    start = time.time()
    grid1.SKI(interpolation='ModifiedSheppard')
    end = time.time()
    print('done in %2.16f seconds' %(end-start))   
    ...
    grid1.K = grid1.Kd[0]
    K_SKI1 = grid1.W.dot((grid1.W.dot(grid1.K)).T).T    + (noise**2)*np.eye(1000)  
